...
<div class="container mt-4">
    <div class="card">
        <div class="card-header">
            <h4 class="mb-0">Edit Cashback Brand</h4>
        </div>
        <div class="card-body">
            <form action="{{ route('admin.cashback-brands.update', $cashbackBrand) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <div class="image-upload-box" onclick="document.getElementById('logo').click()">
                            <img id="logo_preview"
                                 src="{{ $cashbackBrand->logo ? asset($cashbackBrand->logo) : '' }}"
                                 style="{{ $cashbackBrand->logo ? '' : 'display:none;' }}">
                            <svg id="logo_placeholder" style="{{ $cashbackBrand->logo ? 'display:none;' : '' }}" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                        ...
                        @enderror
                        <small class="form-text text-muted d-block mt-2">Leave empty to keep current logo. Max 2MB.</small>
                    </div>

                    <div class="col-md-6 mb-3">
                        <label for="store_name" class="form-label">Store Name <span class="text-danger">*</span></label>
                        <input type="text" class="form-control @error('store_name') is-invalid @enderror" id="store_name" name="store_name" value="{{ old('store_name', $cashbackBrand->store_name) }}" maxlength="255" placeholder="e.g. Best Buy" required>
                        @error('store_name')
                            <div class="text-danger small">{{ $message }}</div>
                        @enderror

                        <label for="cashback_rate" class="form-label mt-3">Cash Back Rate (%) <span class="text-danger">*</span></label>
                        <input type="text" class="form-control @error('cashback_rate') is-invalid @enderror" id="cashback_rate" name="cashback_rate" value="{{ old('cashback_rate', $cashbackBrand->cashback_rate) }}" maxlength="20" placeholder="e.g. 7" required>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control @error('redirect_url') is-invalid @enderror" id="redirect_url" name="redirect_url" value="{{ old('redirect_url', $cashbackBrand->redirect_url) }}" maxlength="2048" placeholder="https://... or /store/...">
                ...
                <div class="mb-3">
                    ...
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" id="status" name="status" value="1" {{ $cashbackBrand->status ? 'checked' : '' }}>
                        <label class="form-check-label" for="status">Active</label>
                    </div>
                </div>

                <div class="mb-3">
                    <button type="submit" class="btn btn-primary">Update Cashback Brand</button>
