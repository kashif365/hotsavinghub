    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex">
                ...
                <a href="{{ route('admin.sliders.create') }}" class="btn btn-primary btn-sm">+ Add New Slider</a>
            </div>
        </div>
        <div class="card-body">
            @if(session('success'))
            ...
            <form id="bulkDeleteForm" method="POST" action="{{ route('admin.sliders.bulkDelete') }}">
                @csrf
                @method('DELETE')
                <input type="hidden" name="ids" id="bulkDeleteIds">
            </form>

            <table id="slidersTable" class="table table-bordered table-striped align-middle w-100">
                <thead>
                    <tr>
                ...
                <tbody id="slidersTableBody">
                    ...
                    <tr data-id="{{ $slider->id }}" data-status="{{ $slider->status }}">
                        <td>
                            <input type="checkbox" name="ids[]" value="{{ $slider->id }}" class="rowCheckbox">
                        </td>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.status-toggle').forEach(toggle => {
        toggle.addEventListener('change', function() {
            ...
            fetch(`/admin/sliders/${id}/update-status`, {
                method: 'PATCH',
                headers: {
